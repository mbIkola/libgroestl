...
class GroestlTest : public HashShitFuncTestTemplate {
public:
    GroestlTest() : HashShitFuncTestTemplate() { digest_size = 32; /* 256 bit */  };
protected:
    string calc(string input) {
        uint8_t *input_binary = hex2bin(input);
        groestl(input_binary, input.size()/2, digest_binary);
        delete[] input_binary;
...
TEST_P(GroestlTest, defaultFixturesTest) {
...
auto groestlFixtures = FixturesCSVLoader::load("data/groestl.csv");
INSTANTIATE_TEST_CASE_P(defaultFixturesTest, GroestlTest, ::testing::ValuesIn(groestlFixtures));
